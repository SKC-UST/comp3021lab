package lab9;

import java.util.List;

/**
 *
 * COMP 3021
 *
This is a class that prints the maximum value of a given array of 90 elements

This is a single threaded version.

Create a multi-thread version with 3 threads:

one thread finds the max among the cells [0,29]
another thread the max among the cells [30,59]
another thread the max among the cells [60,89]

Compare the results of the three threads and print at console the max value.

 *
 * @author valerio
 *
 */
public class FindMax {
	// this is an array of 90 elements
	// the max value of this array is 9999
	static int[] array = { 1, 34, 5, 6, 343, 5, 63, 5, 34, 2, 78, 2, 3, 4, 5, 234, 678, 543, 45, 67, 43, 2, 3, 4543,
			234, 3, 454, 1, 2, 3, 1, 9999, 34, 5, 6, 343, 5, 63, 5, 34, 2, 78, 2, 3, 4, 5, 234, 678, 543, 45, 67, 43, 2,
			3, 4543, 234, 3, 454, 1, 2, 3, 1, 34, 5, 6, 5, 63, 5, 34, 2, 78, 2, 3, 4, 5, 234, 678, 543, 45, 67, 43, 2,
			3, 4543, 234, 3, 454, 1, 2, 3 };

	public static void main(String[] args) {
		//new FindMax().printMax();
		new FindMax().printMaxThreaded();
	}

	public void printMax() {
		// this is a single threaded version
		int max = findMax(0, array.length - 1);
		System.out.println("the max value is " + max);
	}

	public void printMaxThreaded() {
		// this is a multi threaded version
		ThreadMax task1 = new ThreadMax(0, 29); // 0-29
		Thread t1 = new Thread(task1);

		ThreadMax task2 = new ThreadMax(30, 59); // 30-59
		Thread t2 = new Thread(task2);

		ThreadMax task3 = new ThreadMax(60, 89); // 60-89
		Thread t3 = new Thread(task3);
		// 3 thread of find max for 3 ranges
		t1.start();
		t2.start();
		t3.start(); // start all thread
		try {
			t1.join();
			t2.join();
			t3.join(); // wait until all thread is done
		} catch (InterruptedException ie) {
			ie.printStackTrace();
		}

		//int max = findMax(0, array.length - 1);
		int[] maxArray = {task1.getOutput(), task2.getOutput(), task3.getOutput()}; // result of 3 threads
		int max = task1.getOutput();
		for (int e: maxArray){
			max = Math.max(max, e); // find max value from result
		}
		System.out.println("the max value is " + max); // print max value
	}


	/**
	 * returns the max value in the array within a give range [begin,range]
	 *
	 * @param begin
	 * @param end
	 * @return
	 */
	private int findMax(int begin, int end) {
		// you should NOT change this function
		int max = array[begin];
		for (int i = begin + 1; i <= end; i++) {
			if (array[i] > max) {
				max = array[i];
			}
		}
		return max;
	}

	class ThreadMax implements Runnable { // task class for finding max as thread
		private int begin;
		private int end;
		private int output;

		public ThreadMax(int begin, int end) { // set begin and end of function
			this.begin = begin;
			this.end = end;
		}

		public int getOutput() {
			return output; // get output from function
		}

		@Override
		public void run(){
             output = findMax(begin, end); // set output by running find max function
		}
    }

}
